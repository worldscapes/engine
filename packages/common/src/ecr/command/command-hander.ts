...
export namespace ECRCommandEffect {
  export type InferCommandType<T extends ECRCommandEffect> =
    T extends ECRCommandEffect<infer R> ? R : never;
}
...
export interface ECRCommandHandler<
  EffectType extends ECRCommandEffect = ECRCommandEffect
> {
  commandType: string;
  effect: EffectType;
}
