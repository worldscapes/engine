...
export namespace ECRApiTools {
    ...
    export function runTests(description: string, initializer: () => ECRApi): void {
        ...
        describe(description, () => {
            ...
            describe("injectCommands", () => {
                ...
                test("Should handle create-entity command", () => {
                    ...
                    expect(ecr.runSimulationTick().snapshot.entities[0].id !== ecr.runSimulationTick().snapshot.entities[1].id).toBeTruthy();
                });
                ...
                test("Should handle delete-entity command", () => {
                    ...
                    ecr.injectCommands([ removePrototype(new DeleteEntityCommand(entity.id)) ]);
                ...
                test("Should handle update-component command", () => {
                    ...
                    ecr.injectCommands([
                        removePrototype(new UpdateComponentCommand(entity.id, component, updateComponent))
                    ]);
                    const snapshot = ecr.runSimulationTick().snapshot;
                    expect(snapshot.components[entity.id]).toHaveLength(1);
                    expect(snapshot.components[entity.id]).toContainEqual(updateComponent);
                });
                ...
                test("Should handle delete-resource command", () => {
                    ...
                    ecr.runSimulationTick();
                    ecr.injectCommands([
                        removePrototype(new DeleteResourceCommand(tag))
                    ]);
                    const snapshot = ecr.runSimulationTick().snapshot;
            ...
            describe("addCustomCommandHandler", () => {
                ...
                test("Should handle commands return from handler", () => {
                    const handler = createCommandHandler(CreateEntityWithComponentsCommand, (command, store) => {
                    ...
                    expect(snapshot.components[snapshot.entities[0].id]).toEqual(components);
                });
                    ...
                    resultTestQuery = ECRQuery.create({
                        entity: {
                            check: new EntityRequest({
                                testComponent1: new ComponentSelector(ComponentPurposes.CHECK, TestComponent),
                            }),
                            has: new EntityRequest({
                                testComponent1: new ComponentSelector(ComponentPurposes.HAS, TestComponent),
                            }),
                            has_not: new EntityRequest({
                                testComponent1: new ComponentSelector(ComponentPurposes.HAS_NOT, TestComponent),
                            }),
                            write: new EntityRequest({
                                testComponent1: new ComponentSelector(ComponentPurposes.WRITE, TestComponent),
                            }),
                            read: new EntityRequest({
                                testComponent1: new ComponentSelector(ComponentPurposes.READ, TestComponent),
                            }),
                        },
                        resource: {
                            check: new ResourceRequest(ResourcePurposes.CHECK, 'testResource1'),
                            read: new ResourceRequest(ResourcePurposes.READ, 'testResource1'),
                            write: new ResourceRequest(ResourcePurposes.WRITE, 'testResource1'),
                        }
                    });
