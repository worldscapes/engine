...
export namespace ECRStoreTools {
    ...
    export function runTests(description: string, initializer: () => StoreClass): void {

        class TestComponent extends ECRComponent {
            constructor(readonly componentValue: number) { super(); }
        }

        class TestComponent2 extends ECRComponent {
            constructor(readonly componentValue: string) { super(); }
        }

        class TestResource extends ECRResource {
            constructor(readonly resourceValue: number) { super(); }
        }

        class TestResource2 extends ECRResource {
            constructor(readonly resourceValue: string) { super(); }
        }
        ...
        describe(description, () => {
            ...
            describe("updateComponent", () => {
                ...
                test("Should update component", () => {
                    ...
                    expect(store.getSnapshot().components[id]).toContain(updatedComponent);
                });
                ...
                test("Should throw if updating non-existing component", () => {
                    ...
                    const component = removePrototype(new TestComponent(10));
                    const updatedComponent = removePrototype(new TestComponent(15));
                    expect(() => store.updateComponent(id, component, updatedComponent)).toThrow();
                });
            ...
            describe("executeQuery", () => {
                    ...
                    const query = StoreQuery.create({
                        entity: {
                            has: new StoreEntityRequest({
                                test: new StoreComponentSelector(StoreComponentPurposes.HAS, TestComponent)
                            }),
                        },
                        resource: {}
                    });
                    ...
                    const query = StoreQuery.create({
                        entity: {
                            hasNot: new StoreEntityRequest({
                                test: new StoreComponentSelector(StoreComponentPurposes.HAS_NOT, TestComponent)
                            }),
                        },
                        resource: {}
                    });
                ...
                test("Should return correct result for [RETURN] component purpose", () => {
                    ...
                    const query = StoreQuery.create({
                        entity: {
                            get: new StoreEntityRequest({
                                test: new StoreComponentSelector(StoreComponentPurposes.RETURN, TestComponent)
                            }),
                        },
                        resource: {}
                    });
                    ...
                    expect(result).toEqual({
                        entity: {
                            get: [
                                { entityId: 1, test: testSnapshot.components[1].find(el => isTypeOf(el, TestComponent)) },
                                { entityId: 4, test: testSnapshot.components[4].find(el => isTypeOf(el, TestComponent)) }
                            ]
                        },
                        resource: {}
                    });
                    ...
                    const query = StoreQuery.create({
                        entity: {
                            test: new StoreEntityRequest({
                                has: new StoreComponentSelector(StoreComponentPurposes.HAS, TestComponent),
                            })
                        },
                        resource: {
